Status: 
Tags: [[Physics⚛]]
Links:
___
# Formelsammlung
## Geometrie
Winkelbeziehungen am Dreieck (sin,cos,tan); Pythagoras; Fläche und umfang von Kriesen
## Mechanik
> [!Formula]- Dichte
$$\rho=\frac m V$$

> [!Formula]- Gleichförmige Bewegung
$$s=vt; v=s/t; a=0$$ 

> [!Formula]- Gleichmäßig beschleunigteBewegung
$$s=\frac12at^2; v=at; a=konst.$$ insbes. freier Fall: $a=g$

> [!Formula]- Impuls
$$p=mv$$ 

> [!Formula]- Kraft
$$F=ma$$ 

> [!Formula]- Energie: Speicherformen: Lage
$$E=mgh$$ 

> [!Formula]- Energie: Speicherformen: Bewegung
$$E=\frac12mv^2$$

> [!Formula]- Energie: Speicherformen: Spannung
$$E=\frac12Ds^2$$
$Federkonstante: D=\frac{F}{s}$

> [!Formula]- Energie: Arbiet = zugeführte Enerige
$$E=F_ss$$
$F_s :$ Kraftanteil längs des Weges s

> [!Formula]- Kreisbewegung
$$v=\frac{2\pi r}{T}=2\pi rf; f=\frac1T$$
Winkelgeschwindigkeit: $$\omega=2\pi f$$

> [!Formula]- Zentripitalkraft
$$F_z=\frac{mv^2}{r}=m4\pi^2rf^2$$

## Elektrisches und magnetisches Feld/Stromkreise/Induktion
> [!Formula]
$$I=\frac qt$$

> [!Formula]
Bei Ohmishcen Widerständen $U\sim I, U=RI$
Knotenregel, Maschenregel

> [!Formula]- Reihenschaltung
$$R_{ges}=R_1 + R_2 + \cdots $$
$$U_{ges}=U_1 + U_2 + \cdots ;$$

> [!Formula]- Parallelschaltung
$$\frac1R_{ges}=\frac1R_1 + \frac1R_2 + \cdots $$
$$I_{ges}=I_1 + I_2 + \cdots ;$$

> [!Formula]- Elektrische Feldstärke
$$\vec E=\frac{F_{El}}{q}$$


> [!Formula]- Coulomb-Kraft
$$F=\frac{1}{4\pi\varepsilon_0}\cdot \frac{q_1q_2}{r^2}$$

> [!Formula]- Magnetische Flussdichte
$$B=\frac{F_{Magn}}{I\cdot l}$$


> [!Formula]- Lorentz-Kraft
$$F=q\cdot v\cdot B$$

> [!Formula] 
$$F_{El}=F_{Magn}$$
$$\vec E q=qvB$$
$$\frac Udq=qvB$$
$$\rightarrow U_H = dvB$$

> [!Formula]- Homogenes elketrisches Feld
$$U=\frac Eq=\vec E \cdot s=\Delta \varphi$$

> [!Formula] 
$$E =Fs=\vec E q s$$

> [!Formula]- Kondensatoren Flächenladungsdichte
$$\sigma=\frac qA=\varepsilon_0\varepsilon_r\vec E$$

> [!Formula]- Kondensatoren Kapazität
$$C=\frac qU$$

> [!Formula]- Plattenkondensator
$$C=\varepsilon_0\varepsilon_r\frac Ad$$

> [!Formula]- B in einer langen schlanken Spule
$$B= \mu_0\mu_r\frac nlI$$


> [!Formula]- Eigeninduktivität
$$L=\mu_0\mu_r\frac {n^2}{l}A$$

> [!Formula]- Energie im Feld des Kondensators
$$E=\frac12CU^2$$

> [!Formula]- Energie im Feld einer Spule
$$E=\frac12LI^2$$

## [[Elektromagnetische Induktion|Induktion]]
> [!Formula]- Induktionsspannung
$$U_{ind}=-n\dot\Phi$$
$$\Phi=A_sB$$
$$U_{ind}=-L\dot I$$


> [!Formula]- Leiterstück im Magnetfeld
$$U_{ind}=-\dot A_sB=-vlB$$

## Elektrischer Schwingkreis/Mechanische Schwingung
> [!Formula]- Periodendauer im elektrischn Schwingkreis
$$T=2\pi \sqrt{LC}$$

> [!Formula]- Thomische Schwingungsgleichung
$$E_C+E_L=konst.$$
$$\frac 12CU^2+\frac 12 LI^2=konst.$$
$$\frac 12 \frac {Q^2}{C}+\frac 12 LI^2=konst.$$
Ableiten ->
$$\frac 12 \frac {2Q_{(t)}\dot Q_{(t)}}{C}+ \frac 12 L2I_{(t)}\dot I_{(t)}=0$$
$$\frac {Q_{(t)}}{C}+L\dot I_{(t)}=0$$
$$I=\dot Q; \dot I=\ddot Q$$
$$Q_{(t)}+LC\ddot Q_{(t)}=0$$

> [!Formula]- Energiesatz Elektrischer Schwingkreis
$$\frac 12 CU^2 + \frac 12 LI^2 = konst.$$

> [!Formula] 
$$ F=-Ds$$
$$ma = -Ds$$
$$m\ddot s = -Ds$$
$$\ddot s = -\frac Dms=-\omega^2s$$
$$\omega=\sqrt\frac Dm$$
$$T=2\pi\sqrt\frac mD$$

> [!Formula]- Schwingungsgleichung
$$s(t)=\hat s\sin \omega t$$
$$v(t)=\dot s(t) \Rightarrow \hat v = \hat s \omega$$
$$a(t)=\ddot s(t) \Rightarrow \hat a = \hat s \omega^2$$

## [[Mechanische Wellen]]
> [!Formula]- Ausbreitungsgeschwindigkeit von Schall
$$c=330\frac ms$$

> [!Formula]- Ausbreitungsgeschwindigkeit
$$c=\lambda f$$
Phasensprung bei Reflexion am festen Ende

> [!Formula]- Eigenschwingung bei zwei festen oder zwei losen Enden
$$l=k\frac {\lambda}{2}$$

> [!Formula]- Eigenschwingung; ein festes und ein loses Ende
$$l=(2k-1)\frac {\lambda}{4}$$

> [!Formula]- Eigenfrequenzen
$$f_k=k\frac {c}{2l}$$

## Wellenoptik
> [!Formula]- Brechungsgesetz beim Übergang von Medium 1 in Medium 2
$$\frac {\sin \alpha}{\sin \beta} = \frac {c_1}{c_2}=\frac {n_2}{n_1}$$

> [!Formula]- Doppelspalt/Gitter (Maximum)
$$\sin \alpha_k=\frac {k\lambda}g$$
$$\tan \alpha_k=\frac {d_k}{\alpha}$$
Für $\alpha<10°$ -> $\sin\alpha=\tan\alpha$

> [!Formula]- Einzelspalt (Minimum)
$$\sin \alpha_k=\frac {k\lambda}l$$
$$\tan \alpha_k=\frac {d_k}{\alpha}$$

## Quantenphysik
> [!Formula]- Fotoeffekt
$$E=hf=\frac 12 mv^2+E_A$$
$E_A$:Austrittsarbeit

> [!Formula]- Impuls
$$p=\frac {h}{\lambda}$$

> [!Formula]- "Masse" von Quanten
$$m=\frac {hf}{c^2}$$

> [!Formula]- Phasenwinkel eines Psi-Zeigers
$$\phi=\frac {2\pi\dot s}{\lambda}$$

> [!Formula]- De-Broglie-Wellenlänge
$$\lambda_B=\frac hp$$

> [!Formula]- UBR
$$\Delta x\Delta p\approx h$$

## Entropie
> [!Formula]- 
$$\Delta E=T\Delta S$$

___
References:

Created: 2022-04-05 01:04